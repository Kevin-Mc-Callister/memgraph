 ...
 public:
  ...
  class InMemoryAccessor : public Storage::Accessor {
   private:
    ...
    auto CommitWithTicket(CommitArgs const &commit_args, std::unique_lock<std::mutex> commit_serializer)
        -> std::expected<void, StorageManipulationError>;
    auto OrderedLegacyCommit(CommitArgs const &commit_args, std::unique_lock<std::mutex> commit_serializer,
  ...
  auto QuiesceCommits() const -> std::unique_lock<std::mutex>;
