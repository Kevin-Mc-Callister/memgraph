...
auto InMemoryStorage::InMemoryAccessor::CommitWithTicket(CommitArgs const &commit_args,
                                                         std::unique_lock<std::mutex> commit_serializer)
...
auto InMemoryStorage::InMemoryAccessor::PipelinedCommit(CommitArgs const &commit_args, CommitTicket &ticket,
  ...
  try {
    ...
    return OrderedLegacyCommit(
        commit_args, std::unique_lock<std::mutex>{}, ticket, durability_commit_timestamp, nullptr);
  ...
  if (commit_args.two_phase_commit(*replicating_txn)) {
    ...
    return OrderedLegacyCommit(
        commit_args, std::unique_lock<std::mutex>{}, ticket, durability_commit_timestamp, &*replicating_txn);
  }
  {
...
auto InMemoryStorage::QuiesceCommits() const -> std::unique_lock<std::mutex> {
  std::unique_lock guard{commit_mutex_};  // no new mint, hence no new ticket
